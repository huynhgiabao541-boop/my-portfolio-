    ...
    <header className="fixed top-0 inset-x-0 z-50 glass-nav rounded-b-full">
      <div className="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">
        <div className="flex items-center gap-2 font-mono font-bold text-base tracking-wide">
          <div 
            className="h-10 w-[87px] bg-cyan-500 dark:bg-cyan-300 transition-colors"
            style={{
        ...
        <nav className="hidden md:flex items-center gap-1 font-mono text-sm text-slate-600 dark:text-slate-400 p-1 rounded-full bg-slate-200/50 dark:bg-white/5 border border-slate-300/50 dark:border-white/10">
          {navItems.map((item) => (
            <a 
              key={item.id} 
              href={`#${item.id}`}
              ...
              className={`relative px-4 py-1.5 rounded-full transition-colors z-10 ${
                activeSection === item.id ? 'text-slate-900 dark:text-white' : 'hover:text-brand-cyan'
              }`}
            >
